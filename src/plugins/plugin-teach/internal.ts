...
export default function apply(ctx: Context, config: Dialogue.Config) {
    ...
    ctx.on('dialogue/before-fetch', ({
        regexp, answer, question, original,
    }, conditionals) => {
        const query: FilterQuery<Dialogue> = {};
        if (regexp) {
            if (answer !== undefined) query.answer = { $regex: new RegExp(escapeRegExp(answer)) };
            if (question !== undefined) query.question = { $regex: new RegExp(escapeRegExp(original)) };
        } else {
            if (answer !== undefined) query.answer = answer;
            if (question !== undefined) {
                if (regexp === false) query.question = question;
                else {
                    query.$or = [
                        {
                            flag: { $bitsAllClear: DialogueFlag.regexp },
                            question,
                        },
                        {
                            flag: { $bitsAllSet: DialogueFlag.regexp },
                            $or: [
                                { question: { $regex: new RegExp(escapeRegExp(question)) } },
                                { question: { $regex: new RegExp(escapeRegExp(original)) } },
                            ],
                        },
                    ];
                }
