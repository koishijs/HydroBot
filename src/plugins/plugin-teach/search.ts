...
import {
    isPositiveInteger, parseTeachArgs, Dialogue, DialogueTest, DialogueFlag,
} from './database';
...
async function showInfo({ ctx, session }: Dialogue.Argv) {
    const [data, answers, { totalSize, totalCount }] = await Promise.all([
        ctx.database.db.collection('dialogue').aggregate([
            {
                $group: {
        ...
        ctx.database.db.collection('dialogue').count(),
