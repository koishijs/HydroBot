...
import {
    Observed, pick, difference, observe, isInteger, defineProperty, capitalize,
} from 'koishi-utils';
...
declare module 'koishi-core/dist/context' {
    interface EventMap {
        'dialogue/before-fetch'(test: DialogueTest, conditionals?: FilterQuery<Dialogue>[]): void
...
export namespace Dialogue {
    ...
    export async function fromTest(ctx: Context, test: DialogueTest) {
        ...
        ctx.emit('dialogue/before-fetch', test, query.$and);
        console.log(query.$and);
        if (!query.$and.length) delete query.$and;
        const dialogues = (await ctx.database.db.collection('dialogues').find(query).toArray())
    ...
    function addHistory(
        dialogue: Dialogue, type: ModifyType,
        argv: Dialogue.Argv, revert: boolean, target = history,
    ) {
    ...
    export async function create(dialogue: Dialogue, argv: Dialogue.Argv, revert = false) {
        let id: number;
        if (!dialogue.id) {
            const [ldoc] = await argv.ctx.database.db.collection('dialogue').find().sort('_id', -1)
                .limit(1)
                .toArray();
            if (!ldoc) id = 1;
            else id = ldoc._id + 1;
        } else id = dialogue.id;
        const result = await argv.ctx.database.db.collection('dialogue').insertOne({ ...dialogue, _id: id });
        if (!dialogue.id) dialogue.id = result.insertedId;
    ...
    export async function rewrite(dialogues: Dialogue[], argv: Dialogue.Argv) {
        ...
        for (const dialogue of dialogues) {
            tasks.push(argv.ctx.database.db.collection('dialogue').updateOne({ _id: dialogue.id }, { $set: dialogue }));
        }
    ...
    export async function update(dialogues: Observed<Dialogue>[], argv: Dialogue.Argv) {
        ...
        for (const task of data) {
            tasks.push(argv.ctx.database.db.collection('dialogue').updateOne({ _id: task.id }, { $set: task }));
        }
        await Promise.all(tasks);
    ...
    export async function remove(ids: number[], argv: Dialogue.Argv, revert = false) {
        if (!ids.length) return;
        await argv.ctx.database.db.collection('dialogue').deleteMany({ _id: { $in: ids } });
        for (const id of ids) {
...
export function sendResult(argv: Dialogue.Argv, prefix?: string, suffix?: string) {
    const {
        session, options, uneditable, unknown, skipped, updated, target,
    } = argv;
...
export function useFlag(ctx: Context, flag: keyof typeof DialogueFlag) {
    ctx.on('dialogue/before-fetch', (test, conditionals) => {
        if (test[flag] !== undefined) {
            const $bitsAllClear = test[flag];
            const $bitsAllSet = ~test[flag];
            conditionals.push({ $bitsAllClear, $bitsAllSet });
            // TODO did i misunderstood?
