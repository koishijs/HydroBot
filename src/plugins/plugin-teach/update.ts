...
import {
    difference, deduplicate, sleep, pick, isInteger, parseTime, formatTime,
} from 'koishi-utils';
import {
    Dialogue, DialogueFlag, prepareTargets, sendResult, split, RE_DIALOGUES,
} from './database';
import {
...
export default function apply(ctx: Context) {
    ...
    ctx.before('dialogue/execute', (argv) => {
        ...
        const now = Date.now(); const includeTime = parseTime(includeLast); const
            excludeTime = parseTime(excludeLast);
