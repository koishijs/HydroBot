...
import {
    difference, deduplicate, sleep, pick, isInteger, Time,
} from 'koishi-utils';
import {
    Dialogue, prepareTargets, sendResult, split, RE_DIALOGUES,
} from './utils';
import {
    getDetails, formatDetails, formatAnswer, formatQuestionAnswers,
} from './search';
...
export default function apply(ctx: Context) {
    ...
    ctx.on('dialogue/execute', (argv) => {
        ...
        const now = Date.now(); const includeTime = Time.parseTime(includeLast); const
            excludeTime = Time.parseTime(excludeLast);
    ...
    ctx.on('dialogue/detail', ({
        original, answer, flag, _type, _timestamp,
    }, output) => {
...
export async function update(argv: Dialogue.Argv) {
    const {
        app, session, options, target, config,
    } = argv;
