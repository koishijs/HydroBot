...
import {
    equal, split, prepareTargets, RE_DIALOGUES, parseTeachArgs, isPositiveInteger, Dialogue, DialogueFlag, useFlag,
} from '../database';
...
export default function apply(ctx: Context, config: Dialogue.Config) {
    ...
    ctx.on('dialogue/modify', ({ predOverwrite, predecessors }, data) => {
        ...
        } else if (!contain(data.predecessors, predecessors)) data.predecessors = union(data.predecessors, predecessors.map(String));
    });
