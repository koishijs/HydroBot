...
import {
    App, User, Group, TableType, Database, extendDatabase, Context,
} from 'koishi-core';
...
class MongoDatabase {
    ...
    constructor(public app: App, config: Config) {
        this.config = {
            host: 'localhost', port: 27017, name: 'koishi', protocol: 'mongodb', ...config,
        };
    }
    ...
    async start() {
        ...
        if (this.config.prefix) {
            this.db.collection = (
                (func, prefix) => function collection<T extends TableType>(name: T) {
