...
class AdModelTrainer:
    ...
    def generate_matrix(self):
        ...
        for i in range(0, len(AdPredictor.Types)):
            self.splitWords[i] = [list(self.predictor.splitWords(ad))
                                  for ad in self.dataset[i]]
            self.document[i] = [" ".join(sent0)
                                for sent0 in self.splitWords[i]]
